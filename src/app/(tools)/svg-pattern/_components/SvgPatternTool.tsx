...
const PATTERNS = {
  grid: (fg: string, size: number) =>
    `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg"><path d="M ${size} 0 L 0 0 0 ${size}" fill="none" stroke="${fg}" stroke-width="1"/></svg>`,
  dots: (fg: string, size: number) =>
    `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg"><circle cx="${size / 2}" cy="${size / 2}" r="${size / 8}" fill="${fg}"/></svg>`,
  diagonal: (fg: string, size: number) =>
    `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg"><path d="M0 ${size} L${size} 0 Z" fill="none" stroke="${fg}" stroke-width="2"/></svg>`,
  cross: (fg: string, size: number) =>
    `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg"><path d="M${size / 2} 0v${size}M0 ${size / 2}h${size}" fill="none" stroke="${fg}" stroke-width="1"/></svg>`,
  waves: (fg: string, size: number) =>
    `<svg width="${size}" height="${size / 2}" xmlns="http://www.w3.org/2000/svg"><path d="M 0 ${size / 4} Q ${size / 4} 0, ${size / 2} ${size / 4} T ${size} ${size / 4}" fill="none" stroke="${fg}" stroke-width="2"/></svg>`,
  polkaMap: (fg: string, size: number) =>
    `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg"><circle cx="${size / 4}" cy="${size / 4}" r="${size / 8}" fill="${fg}"/><circle cx="${size / 1.33}" cy="${size / 1.33}" r="${size / 6}" fill="${fg}"/></svg>`,
  zigzag: (fg: string, size: number) =>
    `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg"><path d="M 0 ${size / 2} L ${size / 4} 0 L ${size / 2} ${size / 2} L ${size * 0.75} 0 L ${size} ${size / 2}" fill="none" stroke="${fg}" stroke-width="1"/></svg>`,
};
        ...
        <div className="space-y-6 flex flex-col">
          <Card className="border-white/10 bg-card/70 overflow-hidden flex-1 shadow-sm flex flex-col rounded-3xl min-h-[500px]">
            <CardHeader className="bg-background/20 border-b border-white/5 py-4 px-6 flex flex-row items-center justify-between z-20">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  size="sm"
                  onClick={downloadSvg}
                  className="h-8 gap-1.5 text-xs bg-background/50 hover:bg-background/80"
                >
            ...
            >
              ...
              <div
                className="relative z-10 p-6 sm:p-10 w-full max-w-md mx-auto aspect-[16/9] flex flex-col items-center justify-center rounded-3xl shadow-2xl backdrop-blur-md bg-white/5 border border-white/10"
                style={{ pointerEvents: "none" }}
              >
                <h2 className="text-white text-2xl font-bold tracking-tight mb-2 drop-shadow-md">
                  Infinite Scaling
                </h2>
                <p className="text-white/80 font-medium text-sm text-center max-w-xs drop-shadow-sm leading-relaxed">
                  Zero pixelation natively injected directly through vector
                  primitives.
                </p>
              </div>
            </CardContent>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Card className="border-white/10 bg-card/70 shadow-sm rounded-3xl overflow-hidden">
              <CardHeader className="bg-background/20 border-b border-white/5 py-3 px-4 flex flex-row items-center justify-between">
                <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2 border-l-2 border-rose-500 pl-2">
              ...
              <div className="p-0">
                <Textarea
                  readOnly
                  value={cssValue}
                  className="h-[105px] font-mono text-sm leading-relaxed p-4 bg-background/50 border-0 focus-visible:ring-0 resize-none text-rose-400 break-all overflow-y-auto custom-scrollbar"
                />
              </div>
            </Card>
            ...
            <Card className="border-white/10 bg-card/70 shadow-sm rounded-3xl overflow-hidden">
              <CardHeader className="bg-background/20 border-b border-white/5 py-3 px-4 flex flex-row items-center justify-between">
                <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2 border-l-2 border-amber-500 pl-2">
                  Embeddable SVG Element
                </CardTitle>
                <Button
                  variant={copiedSVG ? "default" : "secondary"}
                  size="sm"
                ...
                >
                  {copiedSVG ? (
                    ...
                    <>
                      <Copy className="size-3.5 mr-2" /> Copy &lt;svg&gt;
                    </>
                  )}
                </Button>
              </CardHeader>
              <div className="p-0">
                <Textarea
                  readOnly
                  value={rawSvgBlob}
                  className="h-[105px] font-mono text-[13px] leading-relaxed p-4 bg-background/50 border-0 focus-visible:ring-0 resize-none text-amber-400"
                />
              </div>
            </Card>
          </div>
            ...
            <div className="p-6 space-y-8 overflow-y-auto custom-scrollbar flex-1 relative z-0">
              ...
              <div className="space-y-4">
                ...
                <Slider
                  ...
                  value={fgOpacity}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setFgOpacity(Number(e.target.value))
                  }
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-6 pt-4 border-t border-white/5">
                <div className="space-y-2">
                  <label className="text-xs font-medium text-foreground block uppercase tracking-wider text-muted-foreground">
                    Background base
                  </label>
