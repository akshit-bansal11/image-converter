...
function detectFormat(value: string): ColorFormat {
  ...
  if (v.startsWith("rgba(") || (v.startsWith("rgb(") && !v.startsWith("rgba("))) return "RGBA";
  ...
  if (/^(bg|text|border|ring|fill|stroke)-[a-z]+-\d{2,3}$/.test(v) || /^[a-z]+-\d{2,3}$/.test(v)) return "Tailwind";
...
function cleanTailwindInput(value: string): string {
  const v = value.trim().toLowerCase().replace(/^(bg|text|border|ring|fill|stroke)-/, "");
...
function toTailwindClass(hex: string): string {
  ...
  return best ? `bg-${best} /* nearest match, Δ${Math.round(bestDist)} */` : `bg-[${hex}]`;
}
...
function getOutputPairs(
  ...
  const all = {
    RGBA:     { label: "RGBA",     description: "Red, green, blue, alpha.",                    value: outputs.rgb  },
    HEXA:     { label: "HEX / HEXA", description: "Hex with optional alpha channel.",          value: outputs.hex  },
    HSLA:     { label: "HSLA",     description: "Hue, saturation, lightness, alpha.",           value: outputs.hsl  },
    Tailwind: { label: "Tailwind", description: "Nearest Tailwind utility class.",              value: twClass       },
  };
...
export default function ColorConverterTool() {
  ...
  const twClass = useMemo(() => {
    if (!outputs) return "";
    // Extract raw hex from outputs.hex (strip alpha for lookup)
    const hex = outputs.hex.split("/")[0].trim().toLowerCase();
    return toTailwindClass(hex);
  }, [outputs]);

  const outputPairs = outputs ? getOutputPairs(detectedFormat, outputs, twClass) : [];
        ...
        <CardHeader className="space-y-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1.5 text-sm text-muted-foreground">
                Enter any color — HEX/A, RGB/A, HSL/A, or a Tailwind class. The other three formats are generated automatically.
              </p>
            </div>
          ...
          {hasError && (
            <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-3 text-sm text-red-300">
              Cannot parse color. Try a valid HEX, RGB/A, HSL/A, or Tailwind shade (e.g. <code>violet-600</code>).
            </div>
          )}
        </CardHeader>
      </Card>
